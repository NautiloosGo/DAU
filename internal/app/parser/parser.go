...
func (c *cha) ReadAndFormatPartners(fileName string) {
	...
	for {
		...
		inboxer.date = massDate
		inboxer.dt = massDate[0] + massDate[1]*100 + massDate[2]*10000
		// строку перевожу в инт и загружаю в структуру на место ДАУ
		inboxer.dau, err = strconv.Atoi(record[4])
		if err != nil {
			fmt.Println(err)
		}
